...
\begin{enumerate}
	...
	\item The HLS IP iterates through the nonce range:
		      \begin{enumerate}
			      ...
			      \item If $h < \text{target}$ and this is the first match: stores
			            nonce and hash, sets status to 1
		      \end{enumerate}
...
\subsection{Solution 1: Pipelined with Two-Stage Hash (II=2)}
The nonce hash finalization is split into two inline stages, each
containing a single 32$\times$32 multiply.  This keeps the critical
path under 10\,ns on the Zynq-7020 speed-grade~$-1$, resolving the
setup timing violations (WNS $= -1.4$\,ns) seen with a single-stage
design.

\begin{lstlisting}[style=cstyle]
#pragma HLS PIPELINE II=2
#pragma HLS INLINE          // on each hash stage
\end{lstlisting}
Applied to the main nonce search loop. HLS inserts a pipeline register
between the two multiply stages, achieving an \textbf{Initiation
Interval (II) of~2} --- one new hash result every two clock cycles.

\subsection{Solution 2: Relaxed Pipeline (II=4, Tcl directive)}
An alternative solution overrides the source-level II=2 with a
Tcl directive \texttt{set\_directive\_pipeline -II 4} for lower
resource usage while still providing good throughput.

\subsection{Array Partitioning}
\begin{lstlisting}[style=cstyle]
#pragma HLS ARRAY_PARTITION variable=block_header complete
\end{lstlisting}
Applied in both solutions.  Ensures all block header words
...
\begin{table}[h]
	...
	\begin{tabular}{@{}lcc@{}}
		\toprule
			\textbf{Solution}                   & \textbf{II} & \textbf{Throughput} \\
			\midrule
			Pipelined II=2 (source pragma)       & 2           & $\sim$50 MH/s       \\
			Relaxed pipeline II=4 (Tcl override) & 4           & $\sim$25 MH/s       \\
			\bottomrule
		\end{tabular}
	\end{table}

\noindent (At 100 MHz clock, II=2 yields $\sim$50 million hashes/second.)
